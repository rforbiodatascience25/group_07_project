...
When computing the proportion of zeros, we realize that our data contains no cells with value zero.

```{r}
clean_2 <- read_tsv(file = "../data/clean_data/clean_2.tsv", show_col_types = FALSE)

# Calculate proportion of zeros in each numeric column
zeros <- clean_2 |> 
...
ggplot(zeros_long, aes(x = sample, y = zero_fraction)) +
  ...
  coord_flip() +
  labs(
    title = "Fraction of Zero Log2 LFQ Values per Cell Type",
...
As mentioned earlier, these columns contain next to 0 NA values, so we don't have to drop them.
